...
class Redis{
	...
	public function setup_server($config) {
		...
		try {
			...
			if ( ! $success) {
				echo ('Cache: Redis connection refused. Check the config.');
				return FALSE;
			}

		} catch (RedisException $e) {
			echo ('Cache: Redis connection refused ('.$e->getMessage().')');
			return FALSE;
		}
	 ...
	 */  
	public function save($key , $value , $ttl = 3600 , $type = 0 , $repeat = 0 , $old = 0)  
	{  
		$return = null;  
	...
	public function listPush($list , $value , $direction = 0 , $repeat = 0)
	{
		...
		case 0:
			if ($repeat) {
				...
				if (is_array($value) AND count($value) > 0) { //支持多个数据插入队列
					$count = count($value);
					for ($i = 0; $i < $count; $i++) {
						$return = $this->_redis->lPush($list , $value[$i]);
					}
				}else{
		...
		case 1:
			if ($repeat) {
				...
				if (is_array($value) AND count($value) > 0) { //支持多个数据插入队列
					$count = count($value);
					for ($i = 0; $i < $count; $i++) {
						$return = $this->_redis->rPush($list , $value[$i]);
					}
				}else{
    ...
    public function keyType($keys){
        return $this->_redis->type($keys);
    }
